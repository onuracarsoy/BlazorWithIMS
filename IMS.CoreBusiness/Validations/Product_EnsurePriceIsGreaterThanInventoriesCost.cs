...
namespace IMS.CoreBusiness.Validations
{
    public class Product_EnsurePriceIsGreaterThanInventoriesCost : ValidationAttribute
    {
        ...
        protected override ValidationResult? IsValid(object? value, ValidationContext validationContext)
        {
           var product =  validationContext.ObjectInstance as Product;
            if (product is not null)
            {
                ...
                {
                    return new ValidationResult($"Ürün fiyatı, stok maliyetinden ({TotalInventoriesCost(product).ToString("c")}) büyük olmalıdır.",
                        new List<string>
                        {
        ...
        private bool ValidatePricing(Product product)
        {
            ...
            if(TotalInventoriesCost(product) > product.ProductPrice)
            {
                return false;
            }
